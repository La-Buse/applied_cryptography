...
for key in table_2018:
    # select MF/5000/5000+key
    send([0x00, 0xA4, 0x02, 0x0C]+[0x02, 0x50, key])
    # read binary
    r = send([0x00, 0xB0, 0x00, 0x00, 0x00])
    print(table_2018[key],':',bytes(r).decode("utf8"))
...
send([0x00, 0xA4, 0x00, 0x0C])
# send verify
data, sw1, sw2 = channel.transmit([0x00, 0x20, 0x00, 0x01, 0x00]) #pin1
print('pin 1: {0} tries left'.format(str(sw2 & 0b00001111)))
...
#send verify
data, sw1, sw2 = channel.transmit([0x00, 0x20, 0x00, 0x85, 0x00]) #pin2
print('pin 2: {0} tries left'.format(str(sw2 & 0b00001111)))
...
send([0x00, 0xA4, 0x00, 0x0C])
# send verify
data, sw1, sw2 = channel.transmit([0x00, 0x20, 0x00, 0x02, 0x00]) #puk
print('puk: {0} tries left'.format(str(sw2 & 0b00001111)))